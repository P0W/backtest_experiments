...
class PerformanceDisplayManager:
    ...
    @staticmethod
    def get_trading_activity_table(result: ExperimentResult, symbols_count: int = None) -> List[List[str]]:
        """Generate trading activity table data"""
        data = [
            ["Metric", "Value"],
            ["Total Trades", f"{result.trades_count}"],
            ["Win Rate", f"{result.win_rate:.2f}%"],
            ["Average Trade Length", f"{getattr(result, 'avg_trade_length', 0):.1f} days"],
            ["Even Trades", f"{result.even_trades}"],
        ]
        if symbols_count:
            data.append(["Data Feeds Used", f"{result.num_data_feeds} of {symbols_count}"])
        else:
    ...
    def calculate_professional_score(result: ExperimentResult) -> int:
        ...
        score = 0
        if getattr(result, 'annualized_return', 0) > 12: score += 2
        if result.sharpe_ratio > 1.0: score += 2
        if result.max_drawdown < 20: score += 2
        if result.win_rate > 50: score += 1
        if result.profit_factor > 1.5: score += 1
        if result.max_losing_streak <= 5: score += 1
        return score
    ...
    @classmethod
    def display_single_result_performance(cls, result: ExperimentResult, title: str, params: Dict[str, Any] = None):
        ...
        print(f"\n💰 FINANCIAL PERFORMANCE")
        print(tabulate(cls.get_performance_metrics_table(result), headers="firstrow", tablefmt="grid"))

        # Risk Metrics  
        print(f"\n⚖️ RISK METRICS")
        print(tabulate(cls.get_risk_metrics_table(result), headers="firstrow", tablefmt="grid"))

        # Trading Activity
        print(f"\n📈 TRADING ACTIVITY")
        print(tabulate(cls.get_trading_activity_table(result), headers="firstrow", tablefmt="grid"))

        # Streak Analysis
        print(f"\n🔥 STREAK ANALYSIS")
        print(tabulate(cls.get_streak_analysis_table(result), headers="firstrow", tablefmt="grid"))

        # Parameters (if provided)
        if params:
            print(f"\n🎯 STRATEGY PARAMETERS")
            print(tabulate(cls.get_parameters_table(params), headers="firstrow", tablefmt="grid"))
...
class StatisticsDisplayManager:
    ...
    def display_top_results_comparison(df: pd.DataFrame):
        ...
        display_cols = [
            ...
            "trades_count", "win_rate", "profit_factor", "expectancy",
            "max_winning_streak", "max_losing_streak", "composite_score",
        ]
        
        # Format headers efficiently
        header_map = {
            "total_return": "Tot Ret%", "annualized_return": "Ann Ret%", "sharpe_ratio": "Sharpe",
            "max_drawdown": "Max DD%", "trades_count": "Trades", "win_rate": "Win%",
            "profit_factor": "PF", "expectancy": "Expect₹", "max_winning_streak": "MaxWin",
            "max_losing_streak": "MaxLoss", "composite_score": "Score"
        }
        
        formatted_headers = [header_map.get(col, col.replace("_", " ").title()) for col in display_cols]
    ...
    @staticmethod
    def get_statistics_table(df: pd.DataFrame, columns: List[str], column_names: List[str]) -> List[List[str]]:
        """Generate statistics table for given columns"""
        stats_data = [["Metric", "Mean", "Median", "Std", "Min", "Max", "Best 25%"]]
        
        for col, name in zip(columns, column_names):
            quantile_val = 0.75 if col not in ["max_drawdown", "max_losing_streak", "avg_loss"] else 0.25
            stats_data.append([
                name,
                f"{df[col].mean():.2f}" if "ratio" not in col else f"{df[col].mean():.3f}",
                f"{df[col].median():.2f}" if "ratio" not in col else f"{df[col].median():.3f}",
                f"{df[col].std():.2f}" if "ratio" not in col else f"{df[col].std():.3f}",
                f"{df[col].min():.2f}" if "ratio" not in col else f"{df[col].min():.3f}",
                f"{df[col].max():.2f}" if "ratio" not in col else f"{df[col].max():.3f}",
                f"{df[col].quantile(quantile_val):.2f}" if "ratio" not in col else f"{df[col].quantile(quantile_val):.3f}",
            ])
    ...
    def display_comprehensive_statistics(cls, df: pd.DataFrame):
        ...
        core_cols = ["total_return", "annualized_return", "sharpe_ratio", "max_drawdown"]
        core_names = ["Total Return (%)", "Annualized Return (%)", "Sharpe Ratio", "Max Drawdown (%)"]
        print(tabulate(cls.get_statistics_table(df, core_cols, core_names), headers="firstrow", tablefmt="grid"))

        # Trading Activity Stats
        print(f"\n🎯 TRADING ACTIVITY STATISTICS")
        trading_cols = ["trades_count", "win_rate", "profit_factor", "expectancy"]
        trading_names = ["Total Trades", "Win Rate (%)", "Profit Factor", "Expectancy (₹)"]
        ...
        streak_names = ["Max Winning Streak", "Max Losing Streak", "Average Win (₹)", "Average Loss (₹)"]
    ...
    def display_key_insights(df: pd.DataFrame):
        ...
        print(f"• {strategies_above_market}/{total_strategies} strategies beat 12% annual benchmark ({strategies_above_market/total_strategies*100:.1f}%)")
        print(f"• {strategies_positive_sharpe}/{total_strategies} strategies achieved Sharpe > 1.0 ({strategies_positive_sharpe/total_strategies*100:.1f}%)")
        print(f"• {strategies_low_drawdown}/{total_strategies} strategies kept drawdown < 20% ({strategies_low_drawdown/total_strategies*100:.1f}%)")
...
class SingleStrategyAnalyzer:
    ...
    @staticmethod
    def _display_strategy_overview(result: ExperimentResult, symbols: List[str], start_date, end_date):
        """Display strategy overview section"""
        print(f"\n🎯 STRATEGY OVERVIEW")
        print("=" * 80)
        backtest_days = (end_date - start_date).days
        print(f"📅 Analysis Period: {start_date.strftime('%Y-%m-%d')} to {end_date.strftime('%Y-%m-%d')} ({backtest_days} days)")
        print(f"📈 Universe: {len(symbols)} instruments")
        print(f"💰 Initial Capital: ₹{result.final_value / (1 + result.total_return / 100):,.2f}")
    ...
    def _display_core_performance_with_benchmarks(result: ExperimentResult):
        ...
        perf_data = [
            ...
            ["Annualized Return", f"{getattr(result, 'annualized_return', 0):.2f}%", "12% (Nifty)"],
    ...
    def _display_detailed_financial_breakdown(result: ExperimentResult):
        ...
        print(tabulate(PerformanceDisplayManager.get_performance_metrics_table(result), headers="firstrow", tablefmt="grid"))

    @staticmethod
    def _display_trading_activity_analysis(result: ExperimentResult, symbols_count: int):
        ...
        trade_data = [
            ...
            ["Average Trade Length", f"{getattr(result, 'avg_trade_length', 0):.1f} days", "Strategy Dependent"],
            ["Even Trades (Breakeven)", f"{result.even_trades}", "Should be Low"],
            ["Data Feeds Utilized", f"{result.num_data_feeds}", f"Out of {symbols_count} available"],
        ]
    ...
    def _display_streak_analysis_psychology(result: ExperimentResult):
        ...
        streak_data = [
            ...
            ["Maximum Winning Streak", f"{result.max_winning_streak} trades", "Confidence Builder"],
            ["Maximum Losing Streak", f"{result.max_losing_streak} trades", "< 5 is Good for Psychology"],
            ["Current Win Streak", f"{result.consecutive_wins} trades", "Current Momentum"],
            ["Current Loss Streak", f"{result.consecutive_losses} trades", "Risk Alert if > 3"],
        ]
        print(tabulate(streak_data, headers="firstrow", tablefmt="grid"))

    @staticmethod
    def _display_comprehensive_risk_assessment(result: ExperimentResult, start_date, end_date):
        """Display comprehensive risk assessment"""
        print(f"\n⚖️ COMPREHENSIVE RISK ASSESSMENT")
        
        # Calculate additional risk metrics
        win_loss_ratio = result.avg_win / abs(result.avg_loss) if result.avg_loss != 0 else 0
        backtest_days = (end_date - start_date).days
        expectancy_monthly = getattr(result, 'expectancy', 0) * (result.trades_count / (backtest_days / 30.44)) if backtest_days > 0 else 0
        
        risk_data = [
            ["Risk Metric", "Value", "Risk Level"],
            ["Maximum Drawdown", f"{result.max_drawdown:.2f}%", "Low" if result.max_drawdown < 10 else "Medium" if result.max_drawdown < 20 else "High"],
            ["Sharpe Ratio", f"{result.sharpe_ratio:.3f}", "Excellent" if result.sharpe_ratio > 2 else "Good" if result.sharpe_ratio > 1 else "Poor"],
            ["Win/Loss Ratio", f"{win_loss_ratio:.2f}", "Good" if win_loss_ratio > 1.5 else "Average" if win_loss_ratio > 1 else "Poor"],
            ["Profit Factor", f"{result.profit_factor:.2f}", "Excellent" if result.profit_factor > 2 else "Good" if result.profit_factor > 1.5 else "Poor"],
            ["Monthly Expectancy", f"₹{expectancy_monthly:,.0f}", "Expected monthly profit"],
        ]
    ...
    def _display_strategy_parameters(params: Dict[str, Any]):
        ...
        print(tabulate(PerformanceDisplayManager.get_parameters_table(params), headers="firstrow", tablefmt="grid"))
    ...
    def _display_performance_benchmarking(result: ExperimentResult):
        ...
        benchmark_data = [
            ...
            ["Annual Return", f"{getattr(result, 'annualized_return', 0):.2f}%", f"{nifty_annual:.1f}%", f"{getattr(result, 'annualized_return', 0) - nifty_annual:+.2f}%"],
            ["Sharpe Ratio", f"{result.sharpe_ratio:.3f}", f"{market_sharpe:.1f}", f"{result.sharpe_ratio - market_sharpe:+.3f}"],
            ["Risk (Max DD)", f"{result.max_drawdown:.2f}%", "15-25%", "Lower is Better"],
        ]
    ...
    def _display_professional_assessment(cls, result: ExperimentResult):
        ...
        recommendation = PerformanceDisplayManager.get_professional_recommendation(score)
    ...
    def _generate_performance_insights(result: ExperimentResult) -> List[str]:
        ...
        if getattr(result, 'annualized_return', 0) > 15:
            insights.append("✅ EXCELLENT: Annualized return exceeds market expectations")
        ...
        # Sharpe insights
        if result.sharpe_ratio > 2.0:
            insights.append("✅ EXCELLENT: World-class risk-adjusted returns (Sharpe > 2.0)")
        elif result.sharpe_ratio > 1.0:
        ...
        # Trading insights
        if result.win_rate > 60:
            insights.append("✅ EXCELLENT: High win rate provides psychological comfort")
        elif result.win_rate > 50:
        ...
        if result.max_losing_streak <= 3:
            insights.append("✅ EXCELLENT: Low losing streaks reduce psychological stress")
        elif result.max_losing_streak <= 5:
            insights.append("✅ GOOD: Manageable losing streaks")
        else:
            insights.append("⚠️  CAUTION: High losing streaks may cause emotional trading")
